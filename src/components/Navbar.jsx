...
function Navbar() {
  ...
  return (
    <>
      <nav className=" bg-black w-screen md:p-8 fixed flex justify-between top-0 left-0 z-50">
        <ul
        ...
        >
          {navs.map((nav) => {
            return (
              <li key={nav.name} className="list-item md:my-0 my-7">
                <a onClick={() => SetOpen(false)} className="" href={nav.link}>
                  {nav.name}
                </a>
              </li>
            );
